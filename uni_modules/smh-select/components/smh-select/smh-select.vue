<template>
	<view>
		<view class="select">
			<view class="select1">
				<view class="order">
					<view :class="{cur:curIndex==index,active1:curIndex==index}" v-for="(item,index) in list1"
						:key="index" class="order_1" @click="show(index)">{{item.title}}</view>
				</view>
			</view>
			<view v-if="curIndex==index" v-for="(item,index) in list1" :key="index" class="order1-1"
				style="height: 650rpx; overflow: auto;">
				<view class="order1">
					<view @click="selectItem(index1,index)" :class="{active:item1.status}"
...
<script>
	export default {
		...
		methods: {
			...
			selectItem (i, j) {
				// if(this.list1[j].data[i].status !== true) {
				...
				this.list1[j].data[i].status = !this.list1[j].data[i].status
